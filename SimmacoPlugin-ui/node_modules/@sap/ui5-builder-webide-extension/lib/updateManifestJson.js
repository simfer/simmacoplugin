...
function isHtmlRepoMode(){
	const projectRootName = findProjectRootName(process.cwd());
	const pathToMtaYaml = findMtaIfExists(process.cwd(), projectRootName);
	if (fs.existsSync(pathToMtaYaml)) {
		...
		return oMtaYamlFileContent.parameters && oMtaYamlFileContent.parameters.deploy_mode && 
				oMtaYamlFileContent.parameters.deploy_mode === "html5-repo";
		}
	else {
		return false;
	}
...
function findMtaIfExists(sPath, rootName) {
	let curPath = sPath;
	let maxTries = 4;
	let mtaYamlPath = path.join(curPath, MTA_YAML_FILE_NAME);
	while (path.basename(curPath) !== rootName && maxTries > 0) {
...
function findProjectRootName(sPath) {
	const sUi5YamlPath = path.join(sPath, UI5_YAML_FILE_NAME);
	if (fs.existsSync(sUi5YamlPath)) {
		const oUi5YamlFile = fs.readFileSync(sUi5YamlPath);
		try {
			const oUi5YamlFileContent = yamlParser.safeLoad(oUi5YamlFile);
			// Project name is stored in the ui5.yaml file under metadata -> name
			const projectName = _.get(oUi5YamlFileContent, ["metadata", "name"]);
			if (projectName) {
				return projectName;
			} else {
				throw new Error("The ui5.yaml file does not contain a project name")
			}
		} catch (e) {
			throw new Error("Failed to parse ui5.yaml file due to the following error: " + e);
		}
	} else {
		throw new Error("No ui5.yaml file found");
	}
}
